...
@Component({
  selector: 'app-pulsebp-fourth-stage-chart',
  templateUrl: './pulsebp-fourth-stage-chart.component.html',
  styleUrls: ['./pulsebp-fourth-stage-chart.component.css']
})
export class PulsebpFourthStageChartComponent implements OnInit {
  currentUser: User;
  @ViewChild('PulseBPchart3', { static: true }) chartContainer: ElementRef;
  ...
  public chart: SVGChart; modalRef: NgbModalRef;
  ...
  constructor(private toastr: ToastrService, private chartinputservice: ChartInputService, private authenticationService: AuthService, private data: DataService, public modalService: NgbModal, public timeConverService: XAxisTimeConversionService) {
    ...
    this.baseGraphViewModel.StagesId = 5;
  ...
  CreateChart(pulseDatasetArray: any, bpDatasetArray: any) {
    ...
    svg.append("line").attr("transform", "translate(-90, 185)")
      ...
      .attr("stroke-width", 1)
      .attr("stroke", "black")
      .attr("marker-end", "url(#triangle2A)")
      .attr("marker-start", "url(#triangle1A)");
  ...
  openModal() {
    ...
    if (this.chartTimeCalculation.isTimeExceed === 0) {
      ...
      this.modalPopupData.modelStagename = 'key_FourthStage';
      this.modalPopupData.stageId = 5;
      ...
      this.modalRef.componentInstance.passBackPulse.subscribe((receivedEntry) => {
        ...
        this.inputChartServer.StagesID = 5;
      ...
      this.modalRef.componentInstance.passBackBP.subscribe((receivedEntry) => {
        ...
        //  jsonData = JSON.parse(bpDatalist);
        //  // tslint:disable-next-line:prefer-for-of
        //  for (let i = 0; i < jsonData.length; i++) {

        //    if (jsonData[i].x === this.chartTimeCalculation.halfPlotting) {
        //      jsonData[i].active = 0;
        ...
        //dataArray = {
        //  x: this.chartTimeCalculation.halfPlotting, //this.chartTimeCalculation.xAxisBPonly,
        //  xBP: plottingXBP ,//this.chartTimeCalculation.xAxisBPonly,
        //  LowerBp: this.bpData.lowerbp, UpperBp: this.bpData.upperbp,
        ...
        //  //oldData = JSON.parse(bpDatalist);
        //  //oldData.push(dataArray);
        //  localStorage.setItem('BPData', JSON.stringify(oldData));
        //}
        //else {
        ...
        //if (localStorage.getItem('PulseData')) {
        //  this.pulseList = JSON.parse(localStorage.getItem('PulseData'));
        //}
        //if (localStorage.getItem('BPData')) {
        //  this.bpList = JSON.parse(localStorage.getItem('BPData'));
        //}
  ...
  LoadChartData(baseModal) {

    forkJoin(
      this.chartinputservice.GetPulseData(baseModal),
      this.chartinputservice.GetBPAnalytics(baseModal)
    ).subscribe(([res1, res2]) => {
      if (res1.data != null) {
