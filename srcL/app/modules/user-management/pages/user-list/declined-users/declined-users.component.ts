...
export class DeclinedUsersComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  displayedColumns = ['selectRow','Id', 'LoginId', 'UserName', 'CodeMobilePhone'];
  @Input() dataSource;
  displayExisit = ['selectRow',
    'LoginId', 'UserName', 'CodeMobilePhone'];
  ...
  async getPageSize() {
    ...
    commonCallListViewModel.Action = UserConstant.Declined;
  ...
  setUserCheckBoxData() {
    ...
    this.dataSource.data.filter(x => {
      if (x.IsSeleted == true) iDs += `declined~${x.UserCount}#${x.Id},`;
    });
    if (iDs.length > 0) {
