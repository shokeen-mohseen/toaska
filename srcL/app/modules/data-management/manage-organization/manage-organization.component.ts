...
export class ManageOrganizationComponent implements OnInit, AfterViewInit {
  ...
  actionGroupConfig;
  OrganizationList: string = "OrganizationExport";
  ...
  constructor(private router: Router, private auth: AuthService, public modalService: NgbModal,
    private toastr: ToastrService, private orgnizationService: OrganizationService,
    private manageOrganizationState: ManageOrganizationStateService) { }
  ...
  actionHandler(type) {
    if (type === "add") {
      ...
      if (this.manageOrganizationState.seletedOrganizations == null || this.manageOrganizationState.seletedOrganizations.length <= 0) {
