...
export class AddEditShipfromLocComponent implements OnInit {
  ...
  async GetDistance() {
    ...
    this.tofromLocationViewModel.shipFromLocationId = this.fromlocationId;
    ...
    this.tofromLocationViewModel.shipToLocationId = this.locationId;
    this.customerbylocationService.getDistance(this.tofromLocationViewModel)
      .subscribe(
        result => {
          //this.preferredMaterialList = result.data
          //this.listMaterialLength.emit(this.preferredMaterialList.length);
          this.shipFromMileMappingViewModel.modifiedMiles = result.data[0].distance.toFixed(2);
  ...
  setLocationDropDown(data: any) {
    ...
    this.locationData.sort((x, y) => x.itemName.localeCompare(y.itemName));
  }
  ...
  SaveMilesData() {
    var abc = this.shipFromMileMappingViewModel.modifiedMiles;
