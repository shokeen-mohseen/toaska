...
export class FuelSurchargeComponent implements OnInit, AfterViewInit {
  ...
  actionGroupConfig;
  FuelSurChargeList: string = "FuelSurchargeExport";
  btn: any
  tab1: boolean = true;
  tab2: boolean = false;
  tab3: boolean = false;
  ...
  actionHandler(type) {
    if (type === "add") {
      ...
      if (this.manageFuelChargeStateService.seletedFuelCharges == null || this.manageFuelChargeStateService.seletedFuelCharges.length < 1) {
        this.toastrService.warning("Please select any to edit");
        return;
      } else {
        this.tab1 = true;
        this.tab2 = false;
  ...
  closeTab() {
    if (this.tabGroup.selectedIndex !== 0) {
      this.manageFuelChargeStateService.seletedFuelCharges = [];
    }
