...
export class BulkOrderComponent implements OnInit {
  ...
  validationPallets(Pallets: string) {
    ...
    if (!!Pallets) {
      ...
      if (exced < 0) {
        this.toastrService.error("Pallet quantity should not exceed the truckload quantity. " + this.EquivalentPallates);
        this.isValidationPallets = true;
        return false;
      }
  ...
  calculate() {
    ...
    if (this.dataSourceAddMaterial.data.length > 0 ) {
      ...
      if (sumOfPallets > this.EquivalentPallates) {
        this.toastrService.error("The order quantities exceed truckload capacity.");
        return;
      }
    }
    //.
  ...
  AddMaterial() {
    if (!this.isEditMaterials) {
      ...
      if (this.dataSourceAddMaterial.data.length > 0) {
        ...
        var materialID = data.find(f => f.materialID == this.MaterialId);
        if (!!materialID) {
          return false;
        }
      }
  ...
  VerifyEquipmentMaterialPropertyOrder() {
    ...
    if (this.isEditMaterials) {
      ...
      if (this.SelectMaterialData.propertyValue!=0) {
  ...
  saveBulkOrder() {
    ...
    if (codeStatus == 'SendforShipping') {
      if (!!!this.orderManagement.ShipFromTypeId && !!!this.orderManagement.FromLocationId) {
        this.toastrService.warning("Please Select Ship From");
        return;
      }
    }
    ...
    this.dataSourcefinal.data.map(x => {
      ...
      if (!!x.ScheduledShipDate && !!x.MustArriveByDate) {
        if (x.ScheduledShipDate > x.MustArriveByDate) {
          this.isSubmit = true;
          //.
          this.toastrService.warning('Must Arrive By Date (MABD) should be greater than or equal to Scheduled Ship Date');
        }
      }
  ...
  saveallmaterialProperties() {
    ...
    this.MissingMaterialPropertyData.forEach((value, index) => {
      ...
      if (this.editVerifyEquipmentMaterialProperty.propertyValueUE <= 0) {
        ...
        ErrorMessage = ErrorMessage + "Please enter valid No. pf material units in the Equipment for Material" + value.materialName;
        isErrorFound = true;
      }
    ...
    if (isErrorFound) {
      ...
      this.orderManagementService.SaveMulitpleMaterialProperties(selectedMaterialProperties).subscribe(result => {
        if (result.message == GlobalConstants.Success || result.Message == GlobalConstants.Success) {
          ...
          this.toastrService.error("A technical error occurred. Please contact Tech Support.");
        }
