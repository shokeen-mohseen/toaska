...
import { CommonCallListViewModel, CommonStatusListViewModel, PeriodicElement, RolesViewModel, UserRolesListViewModel, UsersContactViewModel, UsersViewModel, AddressCallListViewModel, UserLocationListViewModel, CommonPaginatorListViewModel } from '..';
...
export class UseraccessService extends BaseService{
  ...
  constructor(private jsonApiService: JsonApiService, private http: HttpClient, preferenceService: PreferenceTypeService
  ) {
  ...
  getUsersStatusList(userModal: CommonPaginatorListViewModel) {
    return this.http.post<any>(`${environment.serverUrl}${routesConstrant.statuslist}`, userModal)
      .pipe(map(user => {
        return user;
      }));
  ...
  saveUserAll(userModal: UsersViewModel[]) {
    return this.http.post<any>(`${environment.serverUrl}${routesConstrant.SaveUserAll}`, userModal)
      .pipe(map(user => {
        return user;
      }));
  }
...
export const routesConstrant = {
  ...
  SaveUserAll: '/usersaccess/SaveUserAll',
}
