...
@Component({
  selector: 'app-cervix-descentchartNew',
  templateUrl: './cervix-descentnew.chart.component.html',
  styleUrls: ['./cervix-descentnew.chart.component.css']
})
export class CervixDescentnewChartComponent implements OnInit {
  ...
  cervixData: CervixChartModalPopup;
  descentData: DescentChartModalPopup;
  public chart: SVGChart;
  modalRef: NgbModalRef;
  modalPopupData: CervixDescentChartWrapper;
  chartTimeCalculation: ChartTimeConversionXAxis; currentUser: User;
  chartMode: string; baseGraphViewModel: BaseChartViewModel;
  constructor(private toastr: ToastrService,private data: DataService,
    private chartinputservice: ChartInputService, private authenticationService: AuthService,
    public modalService: NgbModal, public timeConverService: XAxisTimeConversionService) {
  ...
  ngOnInit(): void {
    ...
    //if (localStorage.getItem('CervixData')) {
    //  this.CervixData = JSON.parse(localStorage.getItem('CervixData'));
    //}
  ...
  createChart(CervixDataset: any, DescentDataset: any) {
    ...
    // TFSID 17031, Rizwan khan, 6 Aug 2020, Add Legend and alert
    ...
    svg.append('text')
      .attr('text-anchor', 'start')
      .attr('class', 'chartlabelname')
      .attr('transform', `translate(${270},100) rotate(-10)`)
      .text('Alert');

    svg.append('text')
      .attr('text-anchor', 'start')
      .attr('class', 'chartlabelname')
      .attr('transform', `translate(${660},110) rotate(-10)`)
      .text('Action');
    ...
    //svg.append('text')
    //  .attr("transform", `translate(${-this.chart.margin.left - 8},${245})`)
    ...
    svg.append('path')
      ...
      .style('stroke', 'black')
      .style('stroke-width', CervixDescentChartXYAxisRange.LineWidth)
      .style('fill', 'none')
      .attr('d', lineCervix);
  ...
  DrawGreenAreaOnChart(svg: any): void {
    ...
    svg.append('line')
      .attr('x1', 0)
      .attr('y1', CervixDescentChartXYAxisRange.GreenChartAreaDivisionLineY1)
      .attr('x2', CervixDescentChartXYAxisRange.GreenChartAreaLineInLoopY1 - 5)
      .attr('y2', 2)
      .attr('stroke-width', CervixDescentChartXYAxisRange.LineWidth)
      .attr('stroke', 'black');
  ...
  //    this.modalRef.componentInstance.passBackCervix.subscribe((receivedEntry) => {
  //      // if (localStorage.getItem('CervixData') === undefined || localStorage.getItem('CervixData') == null) {
  //      //   localStorage.setItem('StartTime', this.chartTimeCalculation.currentDate.toString());
  //      // }
  //      if (localStorage.getItem('CervixData')) {
  //        CervixData = localStorage.getItem('CervixData');
  //      }
  ...
  //      //if (CervixData.length > 0) {
  //      //  let oldData = [];
  //      //  oldData = JSON.parse(CervixData);
  //      //  oldData.push(dataArray);
  //      //  localStorage.setItem('CervixData', JSON.stringify(oldData));
  //      //}
  //      //else {
  //      //  localStorage.setItem('CervixData', JSON.stringify(data));
  //      //}
  ...
  //      //CervixData = localStorage.getItem('CervixData');
  //      //// alert(CervixData)
  //      //this.createChart(CervixData.length > 0 ? JSON.parse(CervixData) : CervixData,
  //      //  DescentData.length > 0 ? JSON.parse(DescentData) : DescentData);

  //    });

  //    this.modalRef.componentInstance.passBackDescent.subscribe((receivedEntry) => {
  //      // if (localStorage.getItem('DescentData') === undefined || localStorage.getItem('DescentData') == null) {
  //      //   localStorage.setItem('StartTime', this.chartTimeCalculation.currentDate.toString());
  //      // }
  //      if (localStorage.getItem('DescentData')) {
  //        DescentData = localStorage.getItem('DescentData');
