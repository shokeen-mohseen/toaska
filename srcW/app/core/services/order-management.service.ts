...
export class OrderManagementService extends BaseService {
  ...
  SaveBulkOrder(data: any[]) {
    return this.http.post<any>(`${environment.coreBaseApiUrl}/SalesOrder/SaveBulkOrder`, data)
      .pipe(map(orderDetails => {
        return orderDetails;
      }));
  }
 
  ApproveAndSendTOMAS(OrderIdsArray: any, SalesOrdertypesArray: any=null) {   
  ...
  GetStatuswithcondition(locationID: number, Credit: string, RequestedDelieveryDate: string = null, OrderID: any=null, OrderTypeID :any=null) {
