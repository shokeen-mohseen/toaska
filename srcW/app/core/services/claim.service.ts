...
export class ClaimService extends BaseService {
  ...
  SetCompleteSetupAndNotify(ClientId: number, ClaimId: number, IsSetupAndNotify: boolean, ClaimStatusId: number, UserId: number): Observable<any> {
    return this.http.get<any>(`${BASE_URL}/Claim/SetCompleteSetupAndNotify?clientId=` + ClientId + '&claimId=' + ClaimId + '&isCompleteSetupAndNotify=' + IsSetupAndNotify + '&claimStatusId=' + ClaimStatusId + '&userId=' + UserId)
      .pipe(map(claimData => {
        return claimData;
      }));
  }
  ...
  SetApprovedClaim(ClientId: number, ClaimId: number, IsApproved: boolean, ClaimStatusId: number, UserId: number): Observable<any> {
    return this.http.get<any>(`${BASE_URL}/Claim/ClaimApproved/?clientId=` + ClientId + '&claimId=' + ClaimId + '&isApproved=' + IsApproved + '&claimStatusId=' + ClaimStatusId + '&userId=' + UserId)
      .pipe(map(claimData => {
        return claimData;
      }));
  }
