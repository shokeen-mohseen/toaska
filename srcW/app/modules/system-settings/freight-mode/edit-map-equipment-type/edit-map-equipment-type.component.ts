...
export class EditMapEquipmentTypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log("freightlist at map equi from service:", this.freightservice.entrylist);
    this.Freightmodelid = this.freightservice.entrylist;
    this.Freightmodelid.map(val => {val.id = val.id });
    console.log("freightlist at map equi after service call:", this.Freightmodelid);
  ...
  onItemSelect() {
    console.log("selectedlist current =", this.selectedItemsA);
  ...
  private Datas(equipmentTypeID): mapequipmenttypefreightmode {
    ...
    console.log("mapequipmenttypefreightmodel", model);
    return model;
  }
  ...
  onfinish() {
    ...
    if (!this.selectedItemsA.length) {
      this.toastr.warning('Please select one equipment');
      return;
    }
    this.dataSource1 = [];
    ...
    this.freightservice.addAllFreightequipmenttype(this.dataSource1).subscribe(x => {
      ...
      const dataForGrid = this.selectedItemsA.map((item, i) => {
        return { ...this.dataSource1[i], equipmentTypeName: item.itemName, id: item.id };
      });
