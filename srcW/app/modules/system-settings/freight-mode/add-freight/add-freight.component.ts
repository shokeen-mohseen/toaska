...
export class AddFreightComponent implements OnInit {
  ...
  addFreight(isSubmit: boolean = false): boolean {
    ...
    if (!isSubmit)
      this.toastr.success('User has been temporary save successfully');
    return true;
  }
  ...
  onSubmit() {
    if (this.addFreight(true) && this.isduplicate == false) {
      ...
      this.freightmodeService.addAllFreight(this.dataSource).subscribe(x => {
        debugger
        this.toastr.success('User has been save successfully');
  ...
  useraddsuccess() {
    ...
    this.freightmodeService.getAll(this.freightViewModel).subscribe((e1: any) => {
      ...
      console.log("freightid",this.freightid);
      console.log("sidelist", this.sidellist);
      console.log("sidellist after slice", this.sidellistlastfive);

      console.log("sidelist name", this.sidellist[(this.sidellist.length - 1)].name);
      console.log("id", this.sidellist[(this.sidellist.length - 1)].id);

      console.log("sidelist after freight created", this.sidellist);
      console.log("sidelistlastfive name", this.sidellistlastfive[(this.sidellistlastfive.length - 1)].name);
  ...
  retrivefreightlist() {
    ...
    this.freightmodeService.getequipfreightIdsbyList(this.freightid).subscribe((result: any) => {
    ...
    console.log("equipforfreight from service", this.freightmodeService.datasourceformattable);
    //  this.equiptypetable();
  }
