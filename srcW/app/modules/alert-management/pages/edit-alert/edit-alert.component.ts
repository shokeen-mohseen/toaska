...
export class EditAlertComponent implements OnInit {
  ...
  constructor(private actRoute: ActivatedRoute, private toster: ToastrService, private formBuilder: FormBuilder, private router: Router, private services: AlertManagementService) { }
  ...
  getUserAlertData(id: number) {
    ...
    this.services.GetUserAlertInformation(id).subscribe(result => {
     
      if (result.Message == "Success" || result.message == "Success") {

        this.SystemEventId = result.data.systemEventsId;
        this.LastUpdate = result.data.updateDateTimeServer + "" + result.updatedBy;
  ...
  setValueInAllControls(userAlertData: UserAlertSmall) {
    ...
    this.alertForm.patchValue({
      ...
      //"SystemEventsID": userAlertData.systemEventsId,
  ...
  BindInternalUsersList(internalUsers: string) {
    ...
    rolesData.forEach((value, index) => {
      this.internalUserList.forEach((innerValue, innerIndex) => {
        if (Number(value) == Number(innerValue.Id)) {
          ...
          this.internalUserMulti.push(value);
        }
      });
    });
    ...
    externalContactData.forEach((value, index) => {
      this.externalUserList.forEach((innerValue, innerIndex) => {
        if (Number(value) == Number(innerValue.id)) {
          ...
          this.externalUserMulti.push(value);
        }
      });
    });
