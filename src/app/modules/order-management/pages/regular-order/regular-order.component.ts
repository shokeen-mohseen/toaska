...
export class RegularOrderComponent implements OnInit, OnDestroy {
  ...
  IsEditNextEnable: boolean = false;
  ...
  public IsChargeUnitDisable(): boolean {

    if (this.orderManagement.adjustChargesRateTypeName != undefined) {
      if (this.orderManagement.adjustChargesRateTypeName == GlobalConstants.PerUnitXPerEA)
        return true;
      else
        return false;
    }
  ...
  get FromLocationChangeByUser(): boolean {
    if (this.SelectedSalesOrdersforEdit == undefined) {
      ...
      if (this.orderManagement != undefined && this.SelectedSalesOrdersforEdit != undefined) {
        ...
        if (Number(this.orderManagement.FromLocationId) === Number(this.SelectedSalesOrdersforEdit.ToLocationId)) {
          return false;
        }
        else {
          return true;
  ...
  get IsUnderReview(): boolean {
    ...
    if (this.OrderStatausCode == "UnderReview") {
      return true;
    }
    else
      return false;
    ...
    if (event.id != 0) {
      ...
      this.orderManagementService.ShipToAddressData(this.commonOrderModel)
        .subscribe(result => {
          ...
          this.addNextButtonAccessiability(false);
  ...
  async BindOrderDetails(orderID, OrderType, item: any = null) {
    ...
    ////this.ShipWithEnableDisable(OrderType, item.Status, item.FromLocationId, item.ToLocationID);
  ...
  adjustmentMaterialGridSelect(item: any) {
    ...
    if (this.orderManagement.adjustChargesRateTypeName != undefined && this.orderManagement.adjustChargesRateTypeName != null && this.orderManagement.adjustChargesRateTypeName == GlobalConstants.PerUnitXPerEA) {

      this.setChargeUnitWithQunatity(this.orderManagement.adjustChargesRateTypeName, this.orderManagement.adjustChargesSectionMaterialID);
    }
  ...
  AdjustmateriachargelDeSelect(item: any) {
    ...
    if (this.orderManagement.adjustChargesRateTypeName != undefined && this.orderManagement.adjustChargesRateTypeName != null) {
      this.setChargeUnitWithQunatity(this.orderManagement.adjustChargesRateTypeName, null);
    }
  }
  ///////////////////////////////////////////////
      ...
      .subscribe(
        result => {
          ...
          if (this.orderManagement.adjustChargesRateTypeName != undefined && this.orderManagement.adjustChargesRateTypeName != null) {
            ...
            if (this.selectedAdjustItems.length > 0)
              this.setChargeUnitWithQunatity(this.orderManagement.adjustChargesRateTypeName, this.selectedAdjustItems[0].id);
            else
              this.setChargeUnitWithQunatity(this.orderManagement.adjustChargesRateTypeName, null);
          }
        }
  ...
  CotractRelatedChargesBind() {
    ...
    this.editNextButtonAccessiability(true);
    this.addNextButtonAccessiability(true);
  ...
  Fromcontract(LocationID: number, RequestDate: any, OrderTypeID: number) {
    this.FromContractlist = [];
    var OrderID = null;
    if (!this.isFromLocationChangeByUser && this.orderManagement != undefined && this.orderManagement.Id != undefined)
      OrderID = this.orderManagement.Id;
  ...
  AddAdjustChargesData() {
    ...
    //console.log(this.orderManagementAdjustCharges.allAdjustChargesData);
    this.dataSourceAdjust = new MatTableDataSource<AdjustChargesModel>(this.orderManagementAdjustCharges.allAdjustChargesData);
  ...
  getChargeUnitData(element: AdjustChargesModel): number {
    var rateType = "";
    var chargeUnit: number = element.chargeUnit;
    if (element.overrideRateTypeID > 0) {
      rateType = element.overrideRateTypeName;
    }
    else {
      rateType = element.rateTypeName;
    }

    if (rateType == "PerUnitXPerEA") {
      if (element.shippedQuantity != 0) {
        chargeUnit = element.shippedQuantity;
      }
      else {
        chargeUnit = element.orderQuantity;
      }
    }
    return chargeUnit;
  }
  ...
  private saveAndVerifyOrder() {
    ...
    this.orderManagementAdjustCharges.allAdjustChargesData.forEach((value, index) => {
      ...
      if (value.rateTypeName != GlobalConstants.PerUnitXPerEA) {
        if (selectedDefault != undefined && value.rateTypeID == selectedDefault.rateTypeID && value.priceMethodID == selectedDefault.priceMethodID && value.rateValue == selectedDefault.rateValue && value.commodityID == selectedDefault.commodityID && value.overrideRateTypeID == 0 && value.overrideRateValue == 0 && value.isModified == false) {
        ...
        //if (value.isModified == false && value.overrideRateValue == 0 && value.rateValue == selectedDefault.rateValue && (value.IsChargeUnitModified == undefined || value.IsChargeUnitModified == false)) {
        if (value.isModified == false && value.overrideRateValue == 0 && selectedDefault != undefined && value.rateValue == selectedDefault.rateValue && (value.IsChargeUnitModified == undefined || value.IsChargeUnitModified == false)) {
        ...
        data => {
          ...
          if (data.data != null) {
            if (this.isSaveEditAndNextOrder == "EditAndNextOrder") {
              this.editNextButtonAccessiability(false);
            }
            ...
            if ((this.isSaveEditAndNextOrder != "SaveAndNextOrder" && this.isSaveEditAndNextOrder != "EditAndNextOrder") || (data.data.status != undefined && (data.data.status == "NewVersion" || data.data.status == "NewDiversion"))) {
              //console.log("after save removing left link .");
              if (!isOrderInEditList) {
              ...
              if (this.CurrentEditListItemIndex == -1)
                this.CurrentEditListItemIndex = this.SalesOrderforEdit.findIndex(x => x.OrderId === this.orderManagement.Id && x.OrderType === this.orderManagement.orderTypeName);


              //console.log("after save get the current index .");
              //console.log(this.CurrentEditListItemIndex);

              //console.log("after save order status" + this.orderManagement.OrderStatusCode);
              //console.log(" after save Credit calculate limit");
              //console.log(this.CreditCalculateLimit());
              //console.log(this.orderManagement.RequestedDeliveryDate);
              //console.log(this.orderManagement.ScheduledShipDate);
  ...
  IsMaterialPallet(materialID: number): boolean {
    if (materialID > 0) {

      if (this.MaterialDetails != undefined && this.MaterialDetails.length > 0) {
        var matavailable = this.MaterialDetails.find(x => Number(x.materialID) == materialID);
        if (matavailable != undefined && matavailable != null) { return false; }
        matavailable = this.ShippingMaterialDetails.find(x => Number(x.materialID) == materialID);
        if (matavailable != undefined && matavailable != null) { return true; }
      }
  ...
  setChargeUnitWithQunatity(RateTypeName: string, materialID: any) {
    if (RateTypeName == GlobalConstants.PerUnitXPerEA) {
      if (materialID != undefined && materialID != null && materialID > 0) {
        if (!this.IsShipment) {
          this.orderManagement.ChargeUnits = this.MaterialDetails.find(x => x.materialID == Number(materialID)).quantity;
        }
        else {
          this.orderManagement.ChargeUnits = this.MaterialDetails.find(x => x.materialID == Number(materialID)).shippedQuantity;
        }
      }
      else {
        var totalQuantity = 0;
        this.MaterialDetails.forEach((value, index) => {
          var exists = this.orderManagementAdjustCharges.allAdjustChargesData.find(x => x.chargeID == this.orderManagement.adjustChargesSectionChargeID && x.materialID == value.materialID);
          if (!(exists != undefined && exists != null)) {
            if (!this.IsShipment) { totalQuantity = totalQuantity + Number(value.quantity) }
            else { totalQuantity = totalQuantity + Number(value.shippedQuantity) }
          }
        });
        this.orderManagement.ChargeUnits = totalQuantity;
  ...
  resetAddAjustment() {
    ...
    this.orderManagement.ChargeUnits = 1;
    this.orderManagement.adjustChargesRateTypeName = "";
    this.orderManagementAdjustCharges.editIndex = -1;
    this.selectadjustChargesSectionChargeID = [];
    this.selectedAdjustItems = [];
  }
        ...
        data => {
          if (data.data.id > 0) {
            ...
            if (!!!this.orderManagement.Id || this.orderManagement.Id == 0) {
              ...
              this.orderManagement.CarrierId = data.data.id;
        ...
        finalize(async () => {
          if (letdata != null || letdata != undefined) {
            ...
            letdata.map(f => {
              ...
              this.orderManagement.orderTypeName = f.OrderType;
              this.orderManagementAdjustCharges.OrderID = Number(this.orderManagement.Id);
              this.orderManagementAdjustCharges.OrderTypeID = this.orderManagement.OrderTypeId;
              this.orderManagementAdjustCharges.OrderTypeCode = this.orderManagement.OrderTypeCode;
              ...
              this.orderManagement.FromContractId = 0;
              this.orderManagement.ToCustomerContractId = 0;
              ...
              this.orderManagement.ShipmentStatus = f.ShipmentStatus;
              this.orderManagement.ShipmentCondition = f.ShipmentCondition;
              ...
              if (this.IsChargeOrder) {
                this.orderManagement.IsDiverted = f.IsDiverted == null ? false : f.IsDiverted;
              }
              else {
                this.orderManagement.IsDiverted = false;
              }

              if (this.IsChargeOrder && this.orderManagement.IsDiverted) {
                this.orderManagementAdjustCharges.IsDiverted = this.orderManagement.IsDiverted;
              }
              else {
                this.orderManagementAdjustCharges.IsDiverted = false;
              }

              //set local storage values
              this.SelectedItem = f;
              localStorage.setItem('OrderStatus', f.OrderStatusCode);
              localStorage.setItem('OrderNumber', f.OrderNumber);
              localStorage.setItem('FromLocationId', f.FromLocationID);
              localStorage.setItem('ToLocationId', f.ToLocationID);
              var SelectedData = JSON.stringify(f);
              localStorage.setItem('Orderdata', SelectedData);


              ////To Disable ship with button
              //this.ShipWithEnableDisable(OrderType, this.orderManagement.OrderStatusCode, this.orderManagement.FromLocationId.toString(), this.orderManagement.ToLocationId.toString());
            ...
            this.ShipWithEnableDisable(this.orderManagement.OrderTypeCode, this.orderManagement.OrderStatusCode, this.orderManagement.FromLocationId.toString(), this.orderManagement.ToLocationId.toString());
            this.EnableDisableCancelBtnOnSelection(this.orderManagement);
  ...
  resetAllSelectedDrodowns() {
    ...
    if (this.MaterialDetails.length > 0) {
      this.MaterialDetails.splice(0, this.MaterialDetails.length);
      this.dataSourceAddMaterial = new MatTableDataSource<PeriodicElement>();
    }
    if (this.ShippingMaterialDetailForUI.length > 0) {
      this.ShippingMaterialDetails.splice(0, this.ShippingMaterialDetails.length);
      this.ShippingMaterialDetailForUI.splice(0, this.MaterialDetails.length);
    ...
    if (this.selectedAdjustItems.length > 0) {
      this.selectedAdjustItems.splice(0, this.selectedAdjustItems.length);
    }

    if (this.MaterialDataForAdjustCharges.length > 0) {
      this.MaterialDataForAdjustCharges.splice(0, this.MaterialDataForAdjustCharges.length);
    }


    if (this.ChargeData.length > 0) {
      this.ChargeData.splice(0, this.ChargeData.length);
    }
    ...
    if (this.orderManagementAdjustCharges.allAdjustChargesData != undefined && this.orderManagementAdjustCharges.allAdjustChargesData.length > 0) {
      ...
      this.orderManagementAdjustCharges.SelectedContractCharges = [];
      this.orderManagementAdjustCharges.TotalAmount = 0;
      this.dataSourceAdjust = new MatTableDataSource<AdjustChargesModel>();
    }

  }


  editNextButtonAccessiability(IsDisable: boolean) {
    this.isEditLoaded = IsDisable;

  }

  addNextButtonAccessiability(IsDisable: boolean) {
    this.isAddLoaded = IsDisable;
  ...
  async refershingDataOrderSelectedOrder() {
    ...
    this.setStatusForMAS();
  }

  setStatusForMAS() {

    if (this.IsShippedAndUnderReview || this.IsUnderReview) {
  ...
  FilterAdjustmentChargeSectionCharges() {
    ...
    if (this.orderManagement.ToContractId != undefined && this.orderManagement.ToContractId > 0) {
      var selectedContract = this.ToContractlist.find(x => x.id == Number(this.orderManagement.ToContractId));

      if (selectedContract != undefined && selectedContract != null && selectedContract.contractNumber.indexOf('Export') != -1) {
        isContractExport = true;
      }
    }

    if (!isContractExport) {
      if (this.SelectedContactCountryCode == undefined || this.SelectedContactCountryCode == null)
        this.SelectedContactCountryCode = "";

      if ((this.SelectedContactCountryCode != "" && this.SelectedContactCountryCode != 'USA' && this.SelectedContactCountryCode != undefined) || (this.SelectedCountryCode != 'USA' && this.SelectedCountryCode != undefined)) {
        isContractExport = true;
      }
    }
  ...
  handleSendForShippingOrderCondition(currentOrderID: number, currentOrderType: string) {
    ...
    if (this.SalesOrderforEdit.length > 1) {
      ...
      if (index > -1 && index < this.SalesOrderforEdit.length - 1) {
        ...
        //console.log("after save remove index");
        ...
        if (index >= 0) {
          //console.log("after save new order open ");
          //console.log(this.SalesOrderforEdit[index].OrderId);
          //console.log(this.SalesOrderforEdit[index].OrderType);
  ...
  EnableDisableCancelBtnOnSelection(row) {
    ...
    if (row != undefined && row != null && row != '') {
      ...
      if ((row.OrderStatusCode == 'Open Order Needs To Be Completed' || row.OrderStatusCode == 'OpenOrderNeedsToBeCompleted' || row.OrderStatusCode == 'Accounting Hold' || row.OrderStatusCode == 'AccountingHold' || row.OrderStatusCode == 'Over Credit Limit' || row.OrderStatusCode == 'OverCreditLimit')
        && (row.OrderCondition == 'New Order' || row.OrderCondition == 'NewOrder') && row.ShipmentId == null && this.isReadAndModifyForBtn && !this.isTruckOrderNotUsed && !this.isOrderSentToMAS && !this.isChargeOrderSentToMAS) {
        //enable cancel btn
        this.buttonBar.enableAction('orderCancel');
        isCancelAllowCount = isCancelAllowCount + 1;
      }
      else if ((row.OrderStatusCode == 'Assigned To Shipment' || row.OrderStatusCode == 'AssignedToShipment') && (row.OrderConditionCode == 'Shipment Planning' || row.OrderConditionCode == 'ShipmentPlanning')
      ...
      else if ((row.OrderStatusCode == 'Assigned To Shipment' || row.OrderStatusCode == 'AssignedToShipment') && this.isReadAndModifyForBtn && !this.isTruckOrderNotUsed && !this.isOrderSentToMAS && !this.isChargeOrderSentToMAS) {
        //enable cancel btn
        this.buttonBar.enableAction('orderCancel');
        isCancelAllowCount = isCancelAllowCount + 1;
      }
  ...
  ShipWithEnableDisable(OrderType: string, Status: string, FromLocationId: string, ToLocationID: string) {

    this.buttonBar.enableAction('shipWith');
    //To Disable ship with button
    if ((OrderType.trim().toLowerCase() != "customer" || OrderType.trim().toLowerCase() != "cpu order" || OrderType.trim().toLowerCase() != "cpuorder" || OrderType.trim().toLowerCase() != "stock transfer" || OrderType.trim().toLowerCase() != "stocktransfer" || OrderType.trim().toLowerCase() != "collections")
      && (Status.trim().toLowerCase() != "open order needs to be completed") && (Status.trim().toLowerCase() != "openorderneedstobecompleted")) {
      this.buttonBar.disableAction('shipWith');
    }
    else if ((OrderType.trim().toLowerCase() != "customer" || OrderType.trim().toLowerCase() != "cpu order" || OrderType.trim().toLowerCase() != "cpuorder") && (FromLocationId == null || FromLocationId == "" || FromLocationId == "0")) {
      this.buttonBar.disableAction('shipWith');
    }
    else if ((OrderType.trim().toLowerCase() != "stock transfer" || OrderType.trim().toLowerCase() != "stocktransfer" || OrderType.trim().toLowerCase() != "collections") && (ToLocationID == null || ToLocationID == "" || ToLocationID == "0")) {
      this.buttonBar.disableAction('shipWith');
    }
  }
