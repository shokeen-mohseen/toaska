...
export class ShipWithOrderComponent implements OnInit {
  ...
  constructor(private router: Router, public activeModal: NgbActiveModal, private orderManagementService: OrderManagementService, private authservices: AuthService,
  ...
  ngOnInit() {
    ...
    if ((orderType.trim().toLowerCase() == "customer" || orderType.trim().toLowerCase() == "cpu order" || orderType.trim().toLowerCase() == "cpuorder" || orderType.trim().toLowerCase() == "stock transfer" || orderType.trim().toLowerCase() == "stocktransfer" || orderType.trim().toLowerCase() == "collections")
      && (orderStatus.trim().toLowerCase() == "open order needs to be completed" || orderStatus.trim().toLowerCase() == "openorderneedstobecompleted")) {
      ...
      if (localStorage.getItem('SelectedTab').toLocaleLowerCase().trim() == 'outbound') {
        if (this.fromLocationId == null || this.fromLocationId == "null" || this.fromLocationId == "") {
          this.toastrService.error(this.getMessage("FromLocationMissing"));//"This order does not have From Location");
        ...
        if (this.toLocationId == null || this.toLocationId == "null" || this.toLocationId == "" || this.toLocationId == "undefined") {
          this.toastrService.error(this.getMessage("ToLocationMissing"));//"This order does not have To-Location");
          this.activeModal.close();
          return false;
        }
        else {
      ...
      this.activeModal.close();
      this.toastrService.error(this.getMessage("ShipWithCanNotBeLinked"));//"This order can not be linked.");
      return false;
    }
  }
  ...
  selectShipToLocation(event) {
    if (event != null) {
      ...
      if (a == -1) {
        this.SalesOrderDatalist = this.AllSalesOrderDatalist;
      }
      else {
        var dd = []
        this.LocationId = a;
    ...
    if (this.SelectedSalesOrderIdsToLink.length > 0) {
      this.orderManagementService.SaveLinkOrder(this.SelectedSalesOrderIdsToLink, this.ParentOrderId, localStorage.getItem('SelectedTab')).subscribe(data => {
        if (data.message == "Success")
          this.InformationMsg = this.getMessage("ShipWithInformation");
        //'Information: Carrier, Equip. No. and Seal No. is/are updated for the linked order(s).';
        this.toastrService.success(this.getMessage("OrdersLinkedSuccessfully"));//"Orders saved successfully.");
  ...
  getMessages() {
    this.messageService.getMessagesByModuleCode("ShipWith", parseInt(localStorage.clientId)).subscribe(
      result => {
        if (result.data) {
