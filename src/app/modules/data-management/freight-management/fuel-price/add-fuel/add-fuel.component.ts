...
export class AddFuelComponent implements OnInit {
  ...
  SelectedFuelPriceToEdit: FuelPriceViewModel;
  ...
  EndDate: Date;
  ngOnInit(): void {
    this.StartDate = new Date(moment().day(1).format('MM/DD/YYYY'));
    ...
    if (this.selectedId && this.isEdit) {
      ...
      this.fuelPriceService.getFuelPriceById(this.fuelPriceViewModel).pipe().subscribe(x => {
          if (x.data) {
            ...
            //console.log(x.data);
            this.SelectedFuelPriceToEdit = x.data[0];
  ...
  ngAfterViewInit(): void {
    this.selectedItemsA = this.locationData.filter(x => x.id == 10);
    this.selectedItemsB = this.UOMData.filter(x => x.id == 220);
    this.selectedItemsC = this.fuelPriceTypeData.filter(x => x.id == 1);
  }
  setFuelPriceControl(viewModel: FuelPriceViewModel) {
    this.settingsUOM = { disabled: true }
    this.settingsFuelPriceType = { disabled: true }
    this.settingsLocation = { disabled: true }
    ...
    this.fuelPriceViewModel.fuelPriceDateTime = new Date(viewModel.fuelPriceDateTime); 
  ...
  setLocationDropDown(data: any) {
    ...
    this.selectedItemsA.push(this.locationData[0]);
    this.fuelPriceViewModel.countryId = this.locationData[0].id;
    this.isCountryId = 0;
  }
  ...
  setUOMDropDown(data: any) {  
    ...
    this.selectedItemsB.push(this.UOMData[0]);
    this.fuelPriceViewModel.uomid = this.UOMData[0].id;
    this.isUomId = 0;
  }
  ...
  setFuelPriceTypeDropDown(data: any) {    
    ...
    this.selectedItemsC.push(this.fuelPriceTypeData[0]);
    this.fuelPriceViewModel.fuelPriceTypeId = this.fuelPriceTypeData[0].id;
  ...
  async saveFuelPrice() {
    ...
    this.fuelPriceViewModel.fuelPriceDateTimePicker = this.convertDatetoStringDate(new Date (this.fuelPriceViewModel.fuelPriceDateTime));
    ...
    if (this.selectedId && this.isEdit) {
      var OldDate = this.convertDatetoStringDate(new Date(this.SelectedFuelPriceToEdit.fuelPriceDateTime));
      var NewDate = this.convertDatetoStringDate(new Date(this.fuelPriceViewModel.fuelPriceDateTime));
      if (OldDate != NewDate)
      {
        ...
        this.fuelPriceService.updateFuelPrice(this.fuelPriceViewModel)
          .subscribe(
            result => {
              ...
              //  this.toastrService.warning(result.data);
  ...
  async checkDuplicate(model: any) {
    var datepicker = model.fuelPriceDateTimePicker;
    ...
    this.fuelPriceViewModel.fuelPriceDateTime = new Date(datepicker);
    this.fuelPriceViewModel.fuelPriceDateTimePicker = datepicker;
  }
