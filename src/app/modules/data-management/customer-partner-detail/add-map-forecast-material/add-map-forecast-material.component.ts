...
export class AddMapForecastMaterialComponent implements OnInit {
  ...
  saveForecastMaterial() {
      ...
      this.customerbylocationService.addCustomerLocationForecastMaterial(objToSave)
        .subscribe(result => {
          if (result.statusCode == 200 && result.data) {
            this.toastrService.success("The records are added successfully.");
          } else {
            this.toastrService.error("Adding Forecast Material failed. Please contact Tech Support.");
          }
    ...
    {
      this.toastrService.error("Select a material to add.");
    }
