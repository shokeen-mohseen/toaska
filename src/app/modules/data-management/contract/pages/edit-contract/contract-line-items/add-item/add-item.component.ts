...
export class AddItemComponent implements OnInit {
  ...
  AddData(form: NgForm) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      models.material = this.gridTemp[i].Material == undefined ? null : this.gridTemp[i].Material;
    ...
    if (res.length != undefined && !datevalidate) {
      this.Data.forEach((value, index) => {
        var obj: ContractLineItemDetails = Object.assign({}, value);
        var d = this.contractcommonservicedata.FinalContractData.lineItems.find(item => ((item.chargeID == obj.chargeID) && (item.materialID == obj.materialID) && item.effectiveStartStr == obj.effectiveStartStr && item.effectiveEndStr == obj.effectiveEndStr))
        if (d) {
          exist = true;          
          return false;
        }
        else {
          this.contractcommonservicedata.FinalContractData.lineItems.push(obj);          
        }
      });
      if (exist) {
        this.toastrService.error('WARNING: Duplicate items cannot be added with same effective start and end dates.');
        return false;
      }
