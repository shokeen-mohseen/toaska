...
export class AddContractComponent implements OnInit {
  ...
  async selected(selectedEdit: ContractViewModel) {
    ...
    await this.CheckLatestVersionExist();
  ...
  CheckLatestVersionExist() {
    this.contractService.getContractLatestExist(this.ContractViewModel)
      .subscribe(async res => {
        if (res.message == "Success") {
          var cnt = parseInt(res.data);
          if (cnt > 0) {
            this.Inactive = true;
            this.IsContractReadWrite = true;
            this.toastrService.warning("Warning: Any modification with this contract is not possible, as the same contract is available with a higher version.");
            return false;
          }
        }
      });
  ...
  Save(source: any) {
    ...
    else if (this.EffectiveStart == null || this.EffectiveStart == undefined) {
      this.toastrService.warning("Please fill all mandatory fields to save the record");
      return false;
    }
    else if (this.EffectiveEnd == null || this.EffectiveEnd == undefined) {
      this.toastrService.warning("Please fill all mandatory fields to save the record");
      return false;
    }
    else if (this.EffectiveEnd < this.EffectiveStart) {
  ...
  onStartDateChange(event) {
     if (event != null) {
      this.contractCommonDataService.ContractStartDate = event;
      this.EffectiveStart = event;
    }
  }

  onEndDateChange(event) {
     if (event != null) {
      this.contractCommonDataService.ContractEndDate = event;
      this.EffectiveEnd = event;
    }
  }
