...
export class EditFreightComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subs.add(this.FMservice.datasourceformattable.subscribe(list => {
      ...
      this.FMservice.count = this.equipforfreight.length;
    }));
  ...
  setupCompleteChange() {
    if (this.FreightViewModel.setupComplete) {
      ...
      this.FreightViewModel.setupCompleteDateTime = this.FMservice.getLocalDateTime(this.convertDatetoStringDate(new Date()));
    } else {
  ...
  onSubmit(formGroup: FormGroup, moveNext = false) {
    ...
    this.FreightViewModel.createDateTimeBrowser = this.FMservice.getLocalDateTime(this.convertDatetoStringDate(new Date()));
    //  this.FreightViewModel.createDateTimeServer = new Date(new Date().toISOString());
    this.FreightViewModel.updateDateTimeBrowser = this.FMservice.getLocalDateTime(this.convertDatetoStringDate(new Date()));
    this.FreightViewModel.updatedDateTimeBrowserStr = this.convertDatetoStringDate(new Date());
    if (this.editMode) {
