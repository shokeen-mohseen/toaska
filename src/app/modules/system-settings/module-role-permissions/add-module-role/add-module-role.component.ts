...
@Component({
  selector: 'app-add-module-role',
  templateUrl: './add-module-role.component.html',
  styleUrls: ['./add-module-role.component.css']
})
    ...
    this.modulerolePermission.CheckExistPermission(temData)
      .subscribe(res => {
        if (res.Message == "Success") {
          if (res.Data == true) {
            this.toastService.error("Selected data already exists.");
            temData.length = 0;
            gridTemp.length = 0;
            return false;
          }
