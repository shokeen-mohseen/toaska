...
export class shipmentManagementComponent implements OnInit, AfterViewInit {
  ...
  openBOLReport() {
    ...
    if (!!!shipments) {
      requestObject.push({ "id": this.shipmentManagementService.EditingShipmentID });
    }
    else {
    if (!this.tabEditData) {
  ...
  openLoadSheet() {
    ...
    var shipments = this.getShipmentForBol();
    if (!!!shipments) {
      requestObject.push({ "id": this.shipmentManagementService.EditingShipmentID, "clientID": this.auth.currentUserValue.ClientId });
    }
    else {
    if (!this.tabEditData) {
      for (var shipment of this.shipmentManagementService.ShipmentsforEdit) {
