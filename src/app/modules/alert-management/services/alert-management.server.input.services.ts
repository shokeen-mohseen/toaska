...
export class AlertManagementService extends BaseService{
  ...
  constructor(private http: HttpClient, private authservices: AuthService, preferenceService: PreferenceTypeService ) {
  ...
  GetAllAlertListForMainPage(paginationModel: UserAlerts) {
    //this.UserAlert.ClientID = this.authservices.currentUserValue.ClientId;
    //this.UserAlert.PageNo = paginationModel.pageNo;
    //this.UserAlert.PageSize = paginationModel.pageSize;

    return this.http.post<any>(BASE_URL + '/useralert/list', paginationModel, { headers: this.headers })
      .pipe(map(DocumentType => {
        return DocumentType;
      }));
  }
