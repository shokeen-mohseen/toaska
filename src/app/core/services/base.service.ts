...
export class BaseService {
  ...
  getLocalDateTime(ServerDate: string) {
    var obj = null;
    if (ServerDate != null && ServerDate != "" && ServerDate != undefined) {
      try {
        obj = new Date(ServerDate.replace('T', ' ') + ' UTC');
      }
      catch (e) {
        console.log(e);
      }
  ...
  getLocalDateTimeFromDateObject(ServerDate: Date) {
    ...
    if (ServerDate != null && ServerDate != undefined) {
      try {
        obj = new Date(this.convertDatetoStringDate(ServerDate) + ' UTC');
      }
      catch (e) {
        console.log(e);
      }
  ...
  async getDefaultNumberOfdaysinShipmentMaterialPlaningReport() {
    if (this.preferenceService.DefaultNumberOfdaysinShipmentMaterialPlaningReport.value < 1) {
      return await this.preferenceService.getPreferenceTypeByCode('DefaultNumberOfdaysinShipmentMaterialPlaningReport').toPromise()
        .then(result => {
          if (result != null && result.data != null) {
            this.preferenceService.DefaultNumberOfdaysinShipmentMaterialPlaningReport.next(parseInt(result.data.preferenceValue));
            return parseInt(result.data.preferenceValue);
          } else {
            // Defaulting it to 10 just in case if the api call fails and we have no response.
            return 10;
    ...
    return this.preferenceService.DefaultNumberOfdaysinShipmentMaterialPlaningReport.value;
  }
