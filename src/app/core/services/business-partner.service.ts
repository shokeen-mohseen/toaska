...
export class BusinessPartnerService extends BaseService {
  ...
  getAllLocationType(clientid: number) {
    var req = {
      ClientID: clientid
    };
    return this.http.post<any>(`${this.MASTER_SERVICE_URL}/LocationType/list`,
      req);
  }

  getLocationOnType(locationids: any) {
    var reqBody = {
      LocationTypeID: locationids
    };
    return this.http.post<any>(`${this.BASE_SERVICE_URL}/Location/GetLocationOnOrders`,
      reqBody);
  }
