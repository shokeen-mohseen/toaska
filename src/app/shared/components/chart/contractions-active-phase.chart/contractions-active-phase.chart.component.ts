...
@Component({
  selector: 'app-contractions-active-phaseChart',
  templateUrl: './contractions-active-phase.chart.component.html',
  styleUrls: ['./contractions-active-phase.chart.component.css']
})
export class ContractionsActivePhaseChartComponent implements OnInit {
  ...
  CreateChart(DataListArray: any) {
    ...
    svg.selectAll('.bar')
      ...
      .attr('fill',
        function (d) {
          if (d.patternValue == '1') {
            return 'url(#greaterThan20LessThan401)'
          }
          if (d.patternValue == '2') {
            return 'url(#greaterThan40_diagonal1)'
          }
          if (d.patternValue == '0') {
            return 'url(#LessThan20Sec_diagonal1)'
          }
        })
      .attr('x', function (d) {
  ...
  LoadContractionData(baseView) {
    this.chartinputservice.GetContractionAnalytics(baseView).subscribe(data => {
      ...
      //console.log("Arrray" + JSON.stringify(this.ContractionData))
      this.CreateChart(this.ContractionData);
    },
      error => {
        // .... HANDLE ERROR HERE 
