...
//                   Latent Phase: Contraction Chart: Merge chart in Lamps 3.0
...
@Component({
  selector: 'app-contractionsChart',
  templateUrl: './contractions.chart.component.html',
  styleUrls: ['./contractions.chart.component.css']
})
export class ContractionsChartComponent implements OnInit {
  ...
  CreateChart(DataListArray: any) {
    ...
    svg.selectAll('.bar')
      ...
      .attr('fill',
        function (d) {
          if (d.patternValue == '1') {
            return 'url(#greaterThan20LessThan40)'
          }
          if (d.patternValue == '2') {
            return 'url(#greaterThan40_diagonal)'
          }
          if (d.patternValue == '0') {
            return 'url(#LessThan20Sec_diagonal)'
          }
        })
      .attr('x', function (d) {
  ...
  openModal() {
    ...
    if (this.chartTimeCalculation.isTimeExceed === 0) {
      ...
      this.modalRef.componentInstance.passEntry.subscribe((receivedEntry) => {
        ...
        this.inputChartServer.XAxisPosition = Number(this.chartTimeCalculation.halfPlotting + 1);//this.chartTimeCalculation.xAxisPosition.toString(); //this.chartTimeCalculation.xAxisPulseBp.toString();
        this.inputChartServer.YAxisPosition =Number(parseInt(receivedEntry.ftListId) + parseInt('1'));
